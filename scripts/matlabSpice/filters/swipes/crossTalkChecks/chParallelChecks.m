...
NCir = 0;
    ...
    for i = coup
        for j = diffr
            for k = neff
                ...
                BaseSimFile = 'ch16Checks_end';
                %BaseSimTHFile = sprintf('%s_C%1.2f_Dr%1.2f_Neff%1.1f_Rad%2.2f_TH_end',base,i,j,k,h);
                BaseSimTHFile = 'ch16Checks_end';
                %BaseSimTriFile = sprintf('%s_C%1.2f_Dr%1.2f_Neff%1.1f_Rad%2.2f_tri_end',base,i,j,k,h);
                BaseSimTriFile = 'ch16Checks_end';
                DataFile = strcat(BaseSimFile,'.1-DC.2d_dat');
                DataTHFile = strcat(BaseSimTHFile,'.1-DC.2d_dat');
                DataTriFile = strcat(BaseSimTriFile,'.1-DC.2d_dat');
                spiceFile = strcat(BaseSimFile,'.sp');
                spiceTHFile = strcat(BaseSimTHFile,'.sp');
                spiceTriFile = strcat(BaseSimTriFile,'.sp');

                NCir;
                PlotAllCrossTalkImvCh8(DataFile,DataTHFile,DataTriFile);
                NCir = NCir +1 ;
                end
    ...
    copyfile(spiceTriFile,spiceCopyLoc);
